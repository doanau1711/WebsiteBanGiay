  ...
  const productApiCalls = useMemo(
    () => ({
      ...
      async create(formData) {
        try {
          // cho key vao mang va dung map duyet cac key trong mang,
          // ma hoa cac key vaf gia tri bang encodeURI(ma hoa cac ki tu dac biet) dam bao an toan khi goi URL thoi
          // key, value ngan cach nhau boi "=", moi cap key- value ngan cach nhau boi dau "&"
          const urlEncodedData = Object.keys(formData)
            .map((key) => encodeURIComponent(key) + '=' + encodeURIComponent(formData[key]))
            .join('&');
          // console.log(urlEncodedData);
